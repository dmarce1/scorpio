...
void GridBlastWave::run(int argc, char* argv[]) {
    ...
    for (int l = 0; l < get_local_node_cnt(); l++) {
        ...
        for (Indexer3d i(BW, GNX - BW - 1); !i.end(); i++) {
            if (!g->zone_is_refined(i[0], i[1], i[2])) {
                ...
                g->radial_avg[ri][1] += (*g)(i).pg(g->X(i));
                g->radial_avg[ri][2] += 2.0 * (*g)(i).ek(g->X(i)) / (*g)(i).rho();
                ;
                g->radial_avg[ri][3] += (*g)(i).ei(g->X(i)) / (*g)(i).rho();
                g->radial_bin_cnt[ri]++;
                j++;
            }
        }
    ...
    tmp = l1;
    MPI_Allreduce(&tmp, &l1, 1, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD );
    l1 /= 8.0 * GRID_DIM * GRID_DIM * GRID_DIM;
    printf("L1 error = %e\n", l1);
    diag_mode = true;
